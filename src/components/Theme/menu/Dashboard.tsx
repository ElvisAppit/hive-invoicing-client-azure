...
import ProductService from '../../../services/Theme/ProductService'
...
const Dashboard = () => {
  ...
  const [productsThisWeek, setProductsThisWeek] = useState<any[]>([])
  const [productsLastWeek, setProductsLastWeek] = useState<any[]>([])
  ...
  const onOrderTabClick = (event: any) => {
    changeDataset(event)
    refreshDataset()
  }

  // const bodyTemplate = (data: any, props: any | ColumnBodyOptions) => {
  //   return (
  //     <>
  //       <span className='p-column-title'>{props.rowIndex}</span>
  //       {data[props.field]}
  //     </>
  //   )
  // }
  const idTemplate = (data: any) => {
    return (
      <>
        <span className='p-column-title'>{data.id}</span>
        {data.id}
      </>
    )
  }
  const categoryTemplate = (data: any) => {
    return (
      <>
        <span className='p-column-title'>{data.category}</span>
        {data.category}
      </>
    )
  }
        ...
        <div className='col-12 xl:col-6'>
          <div className='card'>
            ...
            <DataTable value={products} paginator rows={5} className='p-datatable-products'>
              <Column field='id' header='ID' sortable body={idTemplate}></Column>
              <Column field='category' header='Category' sortable body={categoryTemplate}></Column>
